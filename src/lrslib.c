...
/* linearities are not considered for redundancy */
  ...
  if(Q->debug)
     fprintf (lrs_ofp, "\nnoredundcheck=%ld verifyredund=%ld",Q->noredundcheck, Q->verifyredund);
...
/* other rows need to be tested                                */
  ...
  if(Q->redineq[0] == 0)     /* 2020.9.11 patch, this was set to 1 in readredund but got reset somewhere */
     Q->redineq[0] = 1;
...
lrs_printoutput (lrs_dat * Q, lrs_mp_vector output)
{
  ...
  if (Q->hull || __zero(output[0]))	/*non vertex */
      for (i = 0; i < Q->n; i++)
       {
        ss[i]=cpmp ("", output[i]);
...
/*2019.8.24    bounds for redund */
      if (strcmp (name, "redund") == 0)
        {
        if(!Q->redund && messages &&  overflow != 2)
            lrs_warning (Q,"warning", "\n*switching to redund mode");
     ...
     if (strcmp (name, "project") == 0 || strcmp (name, "eliminate") == 0)     /* fel */
       {
        if(Q->mplrs && Q->hull)
         {
          fprintf (lrs_ofp, "\n\n*%s is an lrs option only for V-representations\n", name);
          return(FALSE);
         }
        ...
        if( Q->nash )
          {
            if(messages &&  overflow != 2)
               fprintf (lrs_ofp, "\n*%s is an lrs/fel option only - skipped\n", name);
          }
        else
          {
            if( messages &&  overflow != 2)
...
/* the other coefficients are multiplied by -2 and shifted one to the right  */
  ...
  if (Q->voronoi)
      ...
      for (i = 1; i <= m; i++)
	{
	  ...
	  __copy(scale, A[i][1]);	/*adjust for scaling to integers of rationals */
	  itomp (ZERO, A[i][0]);
...
/* build the printcobasis string */
	...
	if ( __zero(A[Row[i]][0] ))
	if( ( col == ZERO ) || __zero(A[Row[i]] [col]) )
	  { 
	    nincidence++;
	    if( Q->incidence )
	      {
...
/*          A[i][j]=(A[i][j]*Ars-A[i][s]*A[r][j])/P->det; */
  ...
  __copy(A[r][s], P->det);		/* restore old determinant */
  __copy(P->det, Ars);
...
/* copy data from P to P1    */
  ...
  P1->m_A = P->m_A;
  __copy(P1->det, P->det);
  __copy(P1->objnum, P->objnum);
  __copy(P1->objden, P->objden);
...
/* Suggested new code from db starts */
  ...
  while (i>d){
    while(Cobasic[B[i]]){
      k = d - 1;
      while ((k >= 0) && (__zero(A[Row[i]][Col[k]]) || Cobasic[C[k]])){
       k--;
      }
      if (k >= 0)  {
	...
	/* perform ratio test on rhs or column of basis inverse */
	{
	  ...
	  for (j = start; j < start + degencount; j++)
	      ...
	      else
		{
		  ...
		  else if (__zero(Nmin) && __zero(A[i][ratiocol]))
		    comp = 0;
...
/* assign local variables to structures */
  ...
  for (i = Q->lastdv + 1; i <= m; i++)
    {
      r = Row[i];
      if (__zero(A[r][col]))	/* necessary for lexmin to fail */
	for (j = 0; j < d; j++)
	  {
	    s = Col[j];
	    if (B[i] > C[j])	/* possible pivot to reduce basis */
	      {
		if (__zero(A[r][0]))	/* no need for ratio test, any pivot feasible */
		  {
		    if (!__zero(A[r][s]))
		      return (FALSE);
		  }
...
/* index is cobasic */
  ...
  while ((i <= m) &&
	 (__zero(A[Row[i]][s]) || !__zero(A[Row[i]][0])))
    i++;
         ...
         while( i<= m)
          {
            if(B[i]== remain[k]) 
              {
               j=0;
               while(j+nlinearity<d && (C[j]<= d || __zero(A[Row[i]][Col[j]])))
                  j++;
               if (j+nlinearity<d)
                  {
                    ii=i;
 ...
 */
  ...
  __copy(dest->det, src->det);
  __copy(dest->objnum, src->objnum);
  __copy(dest->objden, src->objden);
...
#ifdef MA
 if(strcmp(Q->fname,"lrs") == 0 || strcmp(Q->fname,"lrsmp") == 0 ||Q->redund || Q->fel)
       try_restart=TRUE;
#endif
...
/* try to restart */
   ...
   if( !pivoting || Q->redund || Q->getvolume || Q->fel || Q->extract)    /* we make restart from original input   */
     {
       overflow = 1L;
...
/* allocate lrs_dat Q, lrs_dic P, read in the problem data and make a copy of P  */
{
  ...
  *Q = lrs_alloc_dat ("LRS globals");    /* allocate and init structure for problem data */
  ...
  strcpy((*Q)->fname,basename(argv[0]));
      ...
      if(stage==1) 
         {
           ...
           if(Q->hull && Q->lponly) /*2022.1.18  find optimizing input rows in V-rep */
        ...
        for(i=0;i<lrs_global_count;i++)     
         {
          ...
          lrs_free_dat(lrs_global_list[0]);
         }
  ...
  for(col=1;col<=d;col++)
    {
      row = -1;
      for(i=1;i<=nlinearity;i++)
	if ((__zero(A[i][d+1]))&& !__zero(A[i][col]))
	  {
	    row = i;
	    break;
	  }
	...
	for(k=1;k<=nlinearity;k++)
	  {
	    if ((__zero(A[k][d+1]))&&(!__zero(A[k][col])) && (k!=row))
	      {
		if (sign(A[k][col])*sign(A[row][col]) < 0)
		  {
		    __copy(A[0][0], A[k][col]);
		    __copy(A[0][1], A[row][col]);
  ...
  for(ind=0;ind<remove[n+1];ind++)
    {
      col=remove[ind];
      j=1;
      while(j <= Q->nlinearity && __zero(P->A[j][col]))
         j++;
      if(j<=Q->nlinearity)
         goto gotcol;
    }
  ...
  for(ind=0;ind<remove[n+1];ind++)
   {
    ...
    for(row = 1; row <= Q->m; row++)
     ...
     if(Q->verbose)
       fprintf(lrs_ofp,"\n*col=%ld minind=%ld pm=%ld size=%ld",col,minind,plus*minus,size);
    }
  ...
  if (skip_col > 0)
      ...
      if (skip_row > 0)
	{
	  for (i = 0; i < skip_row; i++)
		...
		__copy(P1->A[i][j-1], P->A[i][j]);
	      
	    }
	  for (i = skip_row+1; i <= m_A; i++)
	    {
	      for(j = 0; j < skip_col; j++)
		__copy(P1->A[i-1][j], P->A[i][j]);
	      for(j = skip_col+1; j <= d; j++)
		__copy(P1->A[i-1][j-1], P->A[i][j]);
...
void put_linearities_first(lrs_dat *Q, lrs_dic *P)
{
  ...
  for(row=1; row <= nlinearity; row++)
      ...
      if (Q->linearity[row-1] != row)
	{
	  for(i=0;i<=P->d; i++)
	    {
	      __copy(Temp, P->A[row][i]);
	      __copy(P->A[row][i], P->A[Q->linearity[row-1]][i]);
	      __copy(P->A[Q->linearity[row-1]][i], Temp);
	    }
	  __copy(Temp, Q->Gcd[row]);
	  __copy(Q->Gcd[row], Q->Gcd[Q->linearity[row-1]]);
	  __copy(Q->Gcd[Q->linearity[row-1]], Temp);
	  __copy(Temp, Q->Lcm[row]);
	  __copy(Q->Lcm[row], Q->Lcm[Q->linearity[row-1]]);
	  __copy(Q->Lcm[Q->linearity[row-1]], Temp);
...
/*2022.1.17*/
/* the next block of code incorrectly handles the case where the signs */
/* in col are all >0 or <0. In fact the general case handles this correctly     */
/*
  long empty_set;
  empty_set = (!tgroups[Q->m +1] && tgroups[Q->m +2]) ||
              (tgroups[Q->m +1] && !tgroups[Q->m +2]) ||
              (!(tgroups[Q->m +1] + tgroups[Q->m +2]));
  if (empty_set) 
    {
      Q1->m = Q->m;
...
*/
      ...
      for(j = 1; j <= Q->m; j++)
	{
	  if (tgroups[j] < 0) 
		...
		if (tgroups[k] > 0)
		  {
		    __copy(div1, P->A[j][col]);
		    __copy(div2, P->A[k][col]);
		    storesign(div1, POS);
		    __copy(Lcm, div1);
		    lcm(Lcm, div2);
      ...
      for(j=1;j<=Q->m;j++)
	{
	  if (tgroups[j]==0)  /* just copy row, coefficient was '0' */
	    {
	      for(l=0;l<col;l++)
		__copy(P1->A[row][l], P->A[j][l]);
	      for(l=col+1;l<Q->n;l++)
		__copy(P1->A[row][l-1], P->A[j][l]);
	      reducearray(P1->A[row], Q1->n);
	      row++;
	    }
	}
...
long fel_run(lrs_dic *P, lrs_dat *Qin, lrs_restart_dat *R)
{
  ...
  long noredundcheck=Qin->noredundcheck;
  long rounds;
  long min=Qin->m;
  long nin=Qin->n;
  ...
  for (i = 0; i < rounds; i++) /* main loop */
    {
      ...
      Q->debug=debug;
      if(verbose)
        fprintf(lrs_ofp,"\n*beginning iteration %ld",i+1);
      m_begin=Q->m;

      col=lrs_next_col(Q,P,remove);

      if(!Q->mplrs || R->rank==1)
        fprintf(lrs_ofp,"\n*after removing column %ld",col);
      ...
      if (eqn > 0)
	{
	  ...
	  for(j=0;j<=P->d;j++)
	     if ((j!=col) && !__zero(P->A[eqn][j]))
		changesign(P->A[eqn][j]);

	  __copy(div1, P->A[eqn][col]);
	  for(k=1;k<=P->m;k++)
            if (k!=eqn)
              {
		  for(j=0;j<=P->d;j++)
	  ...
	  else     /* eqn <= 0  eliminate inequality */
	{
	  ...
	  for(row=Q->nlinearity+1;row<=Q->m; row++)
              for(l=0;l<=(Q->n)-1;l++)
		__copy(P2->A[row-Q->nlinearity][l], P->A[row][l]);
	  ...
	  for(row=1;row<=Q->nlinearity; row++)
		...
		__copy(P1->A[row][l], P->A[row][l]);
              for(l=col+1;l<=(Q->n)-1;l++)
		__copy(P1->A[row][l-1], P->A[row][l]);
            } 
          for(row=1;row<=P2->m_A; row++)
            {
              Q1->redineq[row+Q->nlinearity]=Q2->redineq[row];
              for(l=0;l<=(Q1->n)-1;l++)
		__copy(P1->A[row+Q->nlinearity][l], P2->A[row][l]);
            }
	  lrs_free_dic(P2 , Q2 );
          lrs_free_dat(Q2 );
...
/*2020.7.16 P,Q contain the new structure incl. redundancies and linearities*/
    ...
    m=Q->m; 
    R->m=m;
    Q->noredundcheck=noredundcheck;
    Q->verbose=verbose;
    if(mplrs)
      {
       if( R->rank==0)       /* boss does no work */
        {
          lrs_free_dic(P1,Q1);
          free(R->redineq);
       ...
       redundmask(Q,R);     /* make the subset of cols to check in mplrs */

    if(debug || (verbose && R->rank==1))
     {
       fprintf(lrs_ofp," \n*Rank=%ld verify=%ld:redineq:\n",R->rank,R->verifyredund);
       for(k=1;k<=m;k++)
