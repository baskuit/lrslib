...
int solve_pointer_constant_sum(game *g, mpq_t **row_payoff_data, lrs_mp_vector row_data, lrs_mp_vector col_data, int payoff_sum_num, int payoff_sum_den)
{
	...
	BuildRepPointerConstantSum(P1, Q1, g, row_payoff_data, 1, 0, payoff_sum_num, payoff_sum_den);
	...
	BuildRepPointerConstantSum(P2orig, Q2, g, row_payoff_data, 0, 1, payoff_sum_num, payoff_sum_den);
	A2orig = P2orig->A;
...
int solve_pointer(game *g, mpq_t **row_payoff_data, mpq_t **col_payoff_data, lrs_mp_vector row_data, lrs_mp_vector col_data)
{
	...
	BuildRepPointer(P1, Q1, g, row_payoff_data, col_payoff_data, 1, 0);
	...
	BuildRepPointer(P2orig, Q2, g, row_payoff_data, col_payoff_data, 0, 1);
	A2orig = P2orig->A;
				 ...
				 lrs_dat *Q2, long *numequilib, lrs_mp_vector output, long linindex[], lrs_mp_vector data)
{
	...
	do
	{
		...
		if (!prune && lrs_getsolution(P2, Q2, output, col))
		{
			...
			if (lrs_nashoutput_(Q2, output, 2L, data))
				(*numequilib)++;
		}
...
long lrs_getfirstbasis2(lrs_dic **D_p, lrs_dat *Q, lrs_dic *P2orig, lrs_mp_matrix *Lin, long no_output, long linindex[])
...
/* no_output is TRUE supresses output headers   */
{
	...
	for (i = 1; i <= m; i++)
		inequality[i] = i;
	if (nlinearity > 0)
	{									 /* some cobasic indices will be removed */
	...
	if (nredundcol > 0)
	{
		...
		*Lin = lrs_alloc_mp_matrix(nredundcol, Qn);
		...
		{
			if (!(Q->homogeneous && Q->hull && i == 0))
			{															 /* skip redund col 1 for homog. hull */
...
/*            and will be removed.                                            */
{
	...
	if (!FirstTime)
	{
		...
		{
			if (linindex[B[i]])
			{ /* pivot out unwanted linearities */
				k = 0;
	...
	{ /* we have not had a successful dictionary built from the given linearities */
...
void FillConstraintRowsPointer(lrs_dic *P, lrs_dat *Q, const game *g, mpq_t **row_payoff_data, mpq_t **col_payoff_data, int p1, int p2, int firstRow)
{
	...
	for (row = firstRow; row < firstRow + g->nstrats[p1]; row++)
	{
		...
		for (t = 0; t < g->nstrats[p2]; t++)
		{
			mpq_set(payoff_rational, p1 == ROW ? (*row_payoff_data[s * g->nstrats[p2] + t]) : *col_payoff_data[t * g->nstrats[p1] + s]);
			mpq_get_num(Num[t + 1], payoff_rational);
			mpz_neg(Num[t + 1], Num[t + 1]);
			mpq_get_den(Den[t + 1], payoff_rational);
		}
...
void FillConstraintRowsPointerConstantSum(lrs_dic *P, lrs_dat *Q, const game *g, mpq_t **row_payoff_data, int p1, int p2, int firstRow, int payoff_sum_num, int payoff_sum_den)
{
	...
	mpq_t payoff_sum;
	mpq_init(payoff_sum);
	mpq_set_si(payoff_sum, payoff_sum_num, payoff_sum_den);
...
void BuildRepPointer(lrs_dic *P, lrs_dat *Q, const game *g, mpq_t **row_payoff_data, mpq_t **col_payoff_data, int p1, int p2)
{
	...
	if (p1 == 0)
	{
		FillConstraintRowsPointer(P, Q, g, row_payoff_data, col_payoff_data, p1, p2, 1);
		FillNonnegativityRows(P, Q, g->nstrats[p1] + 1, g->nstrats[ROW] + g->nstrats[COL], n);
	}
	else
	{
		FillNonnegativityRows(P, Q, 1, g->nstrats[p2], n);
		FillConstraintRowsPointer(P, Q, g, row_payoff_data, col_payoff_data, p1, p2, g->nstrats[p2] + 1); // 1 here
	}
	FillLinearityRow(P, Q, m, n);
...
void BuildRepPointerConstantSum(lrs_dic *P, lrs_dat *Q, const game *g, mpq_t **row_payoff_data, int p1, int p2, int payoff_sum_num, int payoff_sum_den)
{
	...
	if (p1 == 0)
	{
		FillConstraintRowsPointerConstantSum(P, Q, g, row_payoff_data, p1, p2, 1, payoff_sum_num, payoff_sum_den);
		FillNonnegativityRows(P, Q, g->nstrats[p1] + 1, g->nstrats[ROW] + g->nstrats[COL], n);
	}
	else
	{
